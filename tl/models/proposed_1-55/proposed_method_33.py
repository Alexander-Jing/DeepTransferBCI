            ...
            if not self.paras_optim['two_stage']:
                ...
                if self.paras_optim['name'] == 'Adam':
                    ...
                    if self.updating_type in ["entropy_review"]:    
                        ...
                        if self.memory.get_occupancy() >= int(self.capacity/2):
                            entropy_normalized = _entropy_samples_normalized(preds_of_data.detach().clone())
                            C_tensor = torch.tensor(self.num_classes, dtype=torch.float, device=preds_of_data.device)
                            max_entropy = torch.log(C_tensor)
                            _threshold = mean_entropy / max_entropy + self.EnergyAlignment.thre_alpha * std_entropy / max_entropy
                            entropy_avg = torch.mean(entropy_normalized)
                            if entropy_avg >= _threshold:
                                print(f"num instance: {self.num_instance}, not update model in two-stage")
                        ...
                        if self.memory.get_occupancy() >= int(self.capacity/2):
                            ...
                            elif self.losses[1].strip() in ["ConsSamples_selection_two_stage_weighted_4_1_review_4_4","ConsSamples_selection_two_stage_weighted_4_1_review_4_4_1"]: 
