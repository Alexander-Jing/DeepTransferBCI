...
from tl.utils.loss_proposed import MemorySoftplusEnergyAlignment, CE_MDR, PresudoLabelMemorySoftplusEnergyAlignment, MemorySoftplusEnergyWeightedAlignment, \
    MemorySoftplusEnergyRatioSortedAlignment, MemorySoftplusEnergyFeatureWeightedAlignment, MemorySoftplusEnergyWeightedAlignmentMDR, \
        ...
        ConsSamples_selection_two_stage, ConsSamples_selection_two_stage_weighted, ConsSamples_selection_two_stage_weighted_1, ConsSamples_selection_two_stage_weighted_2
        ...
        if len(sup_data) > 0:
            ...
            if not self.paras_optim['two_stage']:
                ...
                if self.paras_optim['name'] == 'Adam':
                    if self.updating_type in ["entropy", "entropy_ema"]:    
                        # first step
                        if self.return_type=='xy':
                            feas_of_data, preds_of_data = self.model(sup_data)
                        elif self.return_type == 'y':
                        ...
                        if self.losses[1].strip() in ["ConsSamples_selection_two_stage_weighted","ConsSamples_selection_two_stage_weighted_1","ConsSamples_selection_two_stage_weighted_2"]: 
...
def loss_prepare(loss_name, EnergyAlignment):
    ...
    elif loss_name in ['ConsSamples_selection_two_stage', 'ConsSamples_selection_two_stage_adaptiveLR', 'ConsSamples_selection_two_stage_adaptiveLR_1']:
