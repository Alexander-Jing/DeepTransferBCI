...
class proposed_TTA(nn.Module):
    ...
    def update_model(self, batch_data, sqrtRefEA):
        ...
        if self.updating_type in ["entropy_review", "entropy_review_1", "entropy_review_2"]:
            ...
            elif self.memory_review in ['get_memory_review_2']:
                review_data, review_data_logits, review_data_class = deepcopy(self.memory.get_memory_review_2(self.batch_size_online, sup_data))
            ...
            if not self.paras_optim['two_stage']:
                ...
                if self.paras_optim['name'] == 'Adam':
                    ...
                    if self.updating_type in ["entropy_review"]:    
                        ...
                        if self.memory.get_occupancy() >= 4*self.batch_size_online:
...
def loss_prepare(loss_name, EnergyAlignment):
    if loss_name == 'MemorySoftplusEnergyAlignment':
        ...
        return ConsSamples_selection_two_stage_weighted_4_1_review_4_1(ratio=EnergyAlignment.ratio, lambda_1=EnergyAlignment.lambda_1, lambda_2=EnergyAlignment.lambda_2, lambda_3=EnergyAlignment.lambda_3, temp=EnergyAlignment.temp, scale=EnergyAlignment.scale, weight_type=EnergyAlignment.weight_type)
