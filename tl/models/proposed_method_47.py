...
class proposed_TTA(nn.Module):
    def __init__(self, model, paras_optim, capacity, num_classes, bn_alpha, temp_factor,
                 ...
                 memory_type='DropMemoryBank_review', memory_review='get_memory_review', result_path=None, sub_information=None, use_buffer_ablation=True):
        ...
        self.sub_information = sub_information
        self.use_buffer_ablation = use_buffer_ablation
        ...
        if not self.paras_optim['two_stage']:
            ...
            if self.loss_name not in ['review_constrastive_loss']:
                self.loss_fn_0 = loss_prepare(loss_name="CE_KL", EnergyAlignment=self.EnergyAlignment)
            else:
                self.loss_fn_0 = loss_prepare(loss_name="ConsSamples_selection_two_stage_weighted_4_1_modified_2", EnergyAlignment=self.EnergyAlignment)
        else:
    ...
    def forward(self, x, sample_test_origin, sqrtRefEA, true_label):
        ...
        for i, data in enumerate(sample_test_origin): 
            ...
            true_label_item = true_label[i].item()
            current_instance = edict(data=data.detach().clone(), prediction=p_l, uncertainty=uncertainty,
                                    logit=out[i].detach().clone(), confidence=conf, time_stamp=self.num_instance, label=true_label_item)  # instance includes the feature, pesudo label, weights, and probability
            
            if self.EnergyAlignment.save_results:
                # save the current instances for visluaization
                self.online_buffer.add_instance(current_instance)
            ...
            if self.num_instance >= self.batch_size_online and self.num_instance % self.update_frequency == 0:
                ...
                if self.EnergyAlignment.save_results:
                    # save the results for visluaization
                    self.online_buffer.save_buffer(path=os.path.join(self.result_path, 'online_visulization_records', f'seed{self.sub_information.seed}_sub_idx{self.sub_information.idx}',
                                                                     f'online_buffer_instance_{self.num_instance}.pt'))
                    self.memory.save_buffer(path=os.path.join(self.result_path, 'memory_visulization_records', f'seed{self.sub_information.seed}_sub_idx{self.sub_information.idx}',
                                                                     f'memory_buffer_instance_{self.num_instance}.pt'))
            ...
            if not self.paras_optim['two_stage']:
                ...
                if self.paras_optim['name'] == 'Adam':
                    ...
                    if self.updating_type in ["entropy_review"]:
                        ...
                        elif self.loss_name in ["CE_KL_review_weighted_10_constrastive_fea"]: 
                            if use_buffer_loss:
                                ...
                                loss = self.loss_fn_0(preds_of_data)
                        elif self.loss_name in ["review_constrastive_loss"]: 
                            if use_buffer_loss:
                                loss = self.loss_fn(preds_of_data, preds_of_data_review, review_data_class, review_data_logits, feas_of_data, feas_of_data_review, mean_entropy, std_entropy)
                            else:
                        ...
                        if self.EnergyAlignment.grad_visual and use_buffer_loss:
                            # 1. 对 loss_sum_1 反向传播
                            ...
                            for p in self.model.parameters():
                                if p.grad is not None:
                                    grads_1.append(p.grad.detach().flatten())
                            ...
                            for p in self.model.parameters():
                                if p.grad is not None:
                                    grads_2.append(p.grad.detach().flatten())
                            grads_2 = torch.cat(grads_2)

                            # 3. 计算模长和余弦相似度
                            norm_1 = grads_1.norm().item()
                            norm_2 = grads_2.norm().item()
                            cosine = F.cosine_similarity(grads_1.unsqueeze(0), grads_2.unsqueeze(0)).item()
                            ...
                            save_dir = os.path.join(
                                self.result_path, 'gradients',
                                f'seed{self.sub_information.seed}_sub_idx{self.sub_information.idx}'
                            )
                        ...
                        if self.EnergyAlignment.save_results_two_stage: 
                            ...
                            self.model.eval()
                            with torch.no_grad():
                                feas_of_data_2, preds_of_data_2 = self.model(sup_data)
                                if not review_data.shape[0] == 0:
                                    feas_of_data_review_2, preds_of_data_review_2 = self.model(review_data)
                                else:
                                    feas_of_data_review_2 = torch.empty((0, feas_of_data_2.shape[1]), device=self.device)
                                    preds_of_data_review_2 = torch.empty((0, self.num_classes), device=self.device)
                            if was_training:
                                self.model.train()

                            save_dir = os.path.join(
                                self.result_path, 'two_stage',
                                f'seed{self.sub_information.seed}_sub_idx{self.sub_information.idx}'
                            )
                            os.makedirs(save_dir, exist_ok=True)
                            save_path = os.path.join(save_dir, f'memory_buffer_instance_{self.num_instance}.pt')
                            ...
                            save_dict = {
                                # model output before second stage updating
                                'feas_of_data': feas_of_data.detach().cpu().clone() if 'feas_of_data' in locals() else None,
                                'preds_of_data': preds_of_data.detach().cpu().clone() if 'preds_of_data' in locals() else None,
                                'feas_of_data_review': feas_of_data_review.detach().cpu().clone() if 'feas_of_data_review' in locals() else None,
                                'preds_of_data_review': preds_of_data_review.detach().cpu().clone() if 'preds_of_data_review' in locals() else None,
                                'review_data_class': review_data_class.detach().cpu().clone() if 'review_data_class' in locals() else None,
                                # model output after second stage updating
                                'feas_of_data_2': feas_of_data_2.detach().cpu().clone(),
                                'preds_of_data_2': preds_of_data_2.detach().cpu().clone(),
                                'feas_of_data_review_2': feas_of_data_review_2.detach().cpu().clone(),
                                'preds_of_data_review_2': preds_of_data_review_2.detach().cpu().clone(),
                            }
                ...
                if self.paras_optim['name'] == 'Adam':
                    ...
                    if self.updating_type in ["entropy_review"]:    
                        ...
                        if self.use_buffer_ablation:
                            use_buffer_loss = self.memory.get_occupancy() >= int(self.capacity/2)
                        else:
                            use_buffer_loss = False
                        ...
                        if use_buffer_loss:
                            ...
                            if self.EnergyAlignment.loss_weight_type in ['buffer_only']:
                                ...
                                loss_1 = self.loss_fn_1_0(preds_of_data_1, feas_of_data_1, preds_of_data.clone().detach())
                            else:
                        ...
                        if self.EnergyAlignment.save_results_two_stage: 
                            ...
                            save_dict = {
                                ...
                                'feas_of_data_review': feas_of_data_review.detach().cpu().clone() if 'feas_of_data_review' in locals() else None,
                                'preds_of_data_review': preds_of_data_review.detach().cpu().clone() if 'preds_of_data_review' in locals() else None,
                                'feas_of_data_1': feas_of_data_1.detach().cpu().clone() if 'feas_of_data_1' in locals() else None,
                                'preds_of_data_1': preds_of_data_1.detach().cpu().clone() if 'preds_of_data_1' in locals() else None,
                                'feas_of_data_review_1': feas_of_data_review_1.detach().cpu().clone() if 'feas_of_data_review_1' in locals() else None,
                                'preds_of_data_review_1': preds_of_data_review_1.detach().cpu().clone() if 'preds_of_data_review_1' in locals() else None,
                                'review_data_class': review_data_class.detach().cpu().clone() if 'review_data_class' in locals() else None,
                                # model output after second stage updating
                                'feas_of_data_2': feas_of_data_2.detach().cpu().clone(),
                                'preds_of_data_2': preds_of_data_2.detach().cpu().clone(),
...
def loss_prepare(loss_name, EnergyAlignment):
    ...
    if loss_name == 'CE_KL':
        ...
        return CE_KL_review_weighted_10_constrastive_visual_PCGrad(lambda_1=EnergyAlignment.lambda_1, lambda_2=EnergyAlignment.lambda_2, lambda_3=EnergyAlignment.lambda_3, temp=EnergyAlignment.temp, scale=EnergyAlignment.scale, confidence_threshold=EnergyAlignment.confidence_threshold, num_classes=EnergyAlignment.num_class, entropy_threshold=EnergyAlignment.entropy_threshold, ratio=EnergyAlignment.ratio, thre_alpha=EnergyAlignment.thre_alpha, ratio_review=EnergyAlignment.ratio_review)
            ...
            return review_constrastive_loss(lambda_1=EnergyAlignment.lambda_1, lambda_2=EnergyAlignment.lambda_2, lambda_3=EnergyAlignment.lambda_3, temp=EnergyAlignment.temp, scale=EnergyAlignment.scale, confidence_threshold=EnergyAlignment.confidence_threshold, num_classes=EnergyAlignment.num_class, entropy_threshold=EnergyAlignment.entropy_threshold, ratio=EnergyAlignment.ratio, thre_alpha=EnergyAlignment.thre_alpha, ratio_review=EnergyAlignment.ratio_review)
