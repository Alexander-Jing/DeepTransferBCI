...
def TTIME(loader, model, args, balanced=True, amdr=False):
    ...
    if not balanced or amdr:
        zk_arrs = np.zeros(args.class_num)
        c = 4
        ...
        if (i + 1) >= args.test_batch and (i + 1) % args.stride == 0:
            ...
            if not balanced or amdr:
                if i + 1 == args.test_batch:
                    args.pred_thresh = 0.35
                    pl = torch.max(softmax_out, 1)[1]
                    # select the presudo label to calculate
                    for l in range(args.test_batch):
                        if softmax_out[l][pl[l]] > args.pred_thresh:
                            zk_arrs[pl[l]] += 1
                else:
                    # update confident prediction ids for current test sample
                    pl = torch.max(softmax_out, 1)[1]
                    if softmax_out[-1][pl[-1]] > args.pred_thresh:
                        zk_arrs[pl[-1]] += 1
...
def train_target(args):
    ...
    if args.balanced:
        acc_t_te, (y_pred, predict, y_true) = TTIME(dset_loaders["Target-Online"], base_network, args=args, balanced=True, amdr=True)  # we also apply the AMDR for testing in the balanced situation
