...
from utils.utils import fix_random_seed, cal_acc_comb, data_loader, cal_auc_comb, cal_score_online, makedir_if_not_exist, build_optimizer
from utils.alg_utils import EA, EA_online
from scipy.linalg import fractional_matrix_power
from models.otta_wimpff import EntropyMinimization
...
# Yuan L, Xie B, Li S. Robust test-time adaptation in dynamic scenarios[C]//Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition. 2023: 15922-15932.
# @Time    : 2025.5.30
# @Author  : Yitao Jing
# @File    : rotta.py
# from github https://github.com/BIT-DA/RoTTA

def eeg_otta_func(loader, model, args, balanced=True):
    # Tent
    ...
    eeg_otta_model = EntropyMinimization(model, args.config, info=None)
    eeg_otta_model.cuda()
    ...
    for i in range(len(loader)):
        ...
        outputs = eeg_otta_model(sample_test)
...
def train_target(args):
    ...
    # for spliting the model in eeg_otta
    ...
    if args.balanced:
        acc_t_te, (y_pred, predict, y_true) = eeg_otta_func(dset_loaders["Target-Online"], base_network, args=args, balanced=True)
        log_str = 'Task: {}, TTA Acc = {:.2f}%'.format(args.task_str, acc_t_te)
    else:
        acc_t_te, (y_pred, predict, y_true) = eeg_otta_func(dset_loaders["Target-Online-Imbalanced"], base_network, args=args, balanced=False)
...
if __name__ == '__main__':
    ...
    if data_name in data_name_list:
        ...
        # hyperparameters for eeg_otta
        # 放在 args 初始化之后、创建 TTA 方法实例之前
        args.config = {
            # 外部评价框架已经执行 IEA，关闭内部对齐
            "alignment": False,

            # FIFO 缓冲区；每接收 32 个新样本进行一次梯度更新
            "buffer_length": 32,

            "optimizer": "Adam",
            "optimizer_kwargs": {
                "lr": 5e-4,
                "beta": 0.9,       # 官方代码将 Adam betas 设置为 (beta, 0.999)
                "weight_decay": 0.0,
            },
        }
