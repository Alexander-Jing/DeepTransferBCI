...
from models.sotta import SoTTA
...
# Gong T, Kim Y, Lee T, et al. Sotta: Robust test-time adaptation on noisy data streams[J]. Advances in Neural Information Processing Systems, 2023, 36: 14070-14093.
# @Time    : 2025/05/26
# @Author  : Yitao Jing
# @File    : sotta.py
# from github https://github.com/Alexander-Jing/DeepTransferBCI.git

def sotta_func(loader, model, args, balanced=True):
    # Tent
    ...
    sotta_model = SoTTA(model, steps=1, args=args)
...
def train_target(args):
    ...
    if args.balanced:
        acc_t_te, (y_pred, predict, y_true) = sotta_func(dset_loaders["Target-Online"], base_network, args=args, balanced=True)
        log_str = 'Task: {}, TTA Acc = {:.2f}%'.format(args.task_str, acc_t_te)
    else:
        acc_t_te, (y_pred, predict, y_true) = sotta_func(dset_loaders["Target-Online-Imbalanced"], base_network, args=args, balanced=False)
...
if __name__ == '__main__':
    ...
    if data_name in data_name_list:
        ...
        # hyperparameters for sotta
        args.weight_decay = 0.0
        args.use_learned_stats = True
