...
from models.rotta import RoTTA
...
def rotta_func(loader, model, args, balanced=True):
    # Tent
    ...
    rotta_model = RoTTA(args, model, optimizer)
    rotta_model.cuda()
...
def train_target(args):
    ...
    if args.balanced:
        acc_t_te, (y_pred, predict, y_true) = rotta_func(dset_loaders["Target-Online"], base_network, args=args, balanced=True)
        log_str = 'Task: {}, TTA Acc = {:.2f}%'.format(args.task_str, acc_t_te)
    else:
        acc_t_te, (y_pred, predict, y_true) = rotta_func(dset_loaders["Target-Online-Imbalanced"], base_network, args=args, balanced=False)
