#!/bin/bash

# Define the workspace folder
workspace_folder="/home/jyt/workspace/DeepTransferBCI"
# Change the current directory to the workspace folder
cd $workspace_folder

# Define the program path (注意: 原脚本中变量名有拼写错误 "progarm_path"，此处保持原样)
progarm_path="tl/proposed_method/ours_debug_m_cls_process_41.py"

# Set the PYTHONPATH environment variable
export PYTHONPATH="${PYTHONPATH}:$(pwd)"

# Define the parameter values to iterate over
confidence_thresholds=(0.55 0.60 0.65 0.70 0.75)
thre_alpha=(0.5 1.0)
loss_weight_type=(10.0)

# Base log path (without the trailing index)
base_log_path="./logs/Baselines-004-test-e300-b64/proposed/proposed_56_BNoff_batch8stride1_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-11-params/proposed_56_BNoff_batch8stride1_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-p"

# Initialize a counter for the parameter combinations
index=1

# Nested loops to iterate over all parameter combinations
for conf in "${confidence_thresholds[@]}"; do
    for ent in "${thre_alpha[@]}"; do
        for los in "${loss_weight_type[@]}"; do
          # Construct the log path with the current index
          log_path="${base_log_path}${index}"
          
          echo "Running combination $index: confidence_threshold=$conf, entropy_threshold=$ent"
          echo "Log path: $log_path"

          # Execute the Python script with the current parameters
          python3 $progarm_path \
            --dataset_name "BNCI2014_004-test" \
            --data_save "True" \
            --data_path "./data/" \
            --data_path_MI "None" \
            --log_path "$log_path" \
            --use_pretrained_model "True" \
            --finetune "False" \
            --momentum "False" \
            --momentum_param "0.5" \
            --gpu_idx "1" \
            --align "True" \
            --batch_size "64" \
            --lr "0.001"  \
            --lr_online "0.001" \
            --epoch "300"  \
            --backbone "EEGNet-4,2" \
            --param_runs "./runs_debug/" \
            --use_BN "False" \
            --stride "1" \
            --batch_size_online "8" \
            --loss_func "CE_KL_review_weighted_10,ConsSamples_selection_two_stage_weighted_4_1_review_4_4_2" \
            --selection_ratio "0.75" \
            --selection_ratio_review "0.25" \
            --mt "0.9" \
            --updating_type "entropy_review" \
            --loss_weights "1.0, 1.0, 1.0" \
            --scale "$los" \
            --confidence_threshold "$conf" \
            --entropy_threshold "0.50" \
            --memory_type "DropMemoryBank_review_8" \
            --memory_review "get_memory" \
            --weight_type "entropy" \
            --memory_capacity "64" \
            --thre_alpha "$ent" \
            --loss_weight_type "gate" \
            --gate_type "mean" \
            --buffer_selefction_type "confidence" \
          # Increment the index for the next combination
          ((index++))
          
          echo "Completed combination $((index-1))"
          echo "----------------------------------------"
        done
    done
done

echo "All parameter combinations have been processed."