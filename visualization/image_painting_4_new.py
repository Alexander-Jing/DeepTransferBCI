...
def generate_zscore_accuracy_no_border_plot_1(save_dir):
    ...
    # 数据集 1: BNCI-2a (Average)
    acc_2a = [0.7631, 0.6857, 0.6214]
    count_2a = [21.8, 27.6, 22.7]
    
    # 数据集 2: BNCI-2b (Average)
    acc_2b = [0.8246, 0.7899, 0.7431]
    count_2b = [22.2, 12.4, 4.8]
    
    # 数据集 3: SHU-3C (Average)
    acc_shu = [0.7949, 0.7348, 0.6551]
    count_shu = [30.6, 45.5, 35.9]
    ...
    datasets = [
        {'name': 'BCI-IV 2a', 'acc': acc_2a, 'count': count_2a, 'color': '#377eb8', 'marker': 'o', 'ls': '-'},
        {'name': 'BCI-IV 2b', 'acc': acc_2b, 'count': count_2b, 'color': '#e41a1c', 'marker': 's', 'ls': '--'},
...
if __name__ == '__main__':
    ...
    if data_name in data_name_list:
        ...
        if data_name in ["BNCI2014001-4-all"]:
            args.result_dir = '/data/datasets_Jyt/DeepTransferBCI/logs_backup/Baselines-001-test-e300-b64-debugs/ours_debug_m_cls_process_1-BNCI2014001-4-all-EEGNet-4,2-e300-b64-new/proposed_57_BNoff_batch8stride8_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-3-params-2-visualization/proposed_57_BNoff_batch8stride8_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-p1'
        elif data_name in ["BNCI2014_004-test"]:
            args.result_dir = '/data/datasets_Jyt/DeepTransferBCI/logs_backup/Baselines-004-test-e300-b64/proposed_new/proposed_57_BNoff_batch8stride1_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-3-params-visualization/proposed_57_BNoff_batch8stride1_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-p6'
        elif data_name in ["WBCIC-SHU-3C"]:
            args.result_dir = '/data/datasets_Jyt/DeepTransferBCI/logs_backup/Baselines-WBCIC-SHU-3C-e300-b64/proposed-new/proposed_57_BNoff_batch8stride1_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-3-params-visualization/proposed_57_BNoff_batch8stride1_CE_KL_review_ConsSamples_selection_two_stage_weighted_4_1_double_lr0.001-p1'
        ...
        for s in [2]:
            ...
            Safe (Z<=0): Acc = 0.7631 ± 0.1187 (avg count = 21.8)
            Warning (0<Z<=0.5): Acc = 0.6857 ± 0.1271 (avg count = 27.6)
            Critical (Z>0.5): Acc = 0.6214 ± 0.1067 (avg count = 22.7)

            "BNCI2014_004-test"
            --- Average Binning Results across 9 Subjects ---
            Safe (Z<=0): Acc = 0.8246 ± 0.1353 (avg count = 22.2)
            Warning (0<Z<=0.5): Acc = 0.7899 ± 0.1548 (avg count = 12.4)
            Critical (Z>0.5): Acc = 0.7431 ± 0.1053 (avg count = 4.8)

            "WBCIC-SHU-3C"
            --- Average Binning Results across 11 Subjects ---
            Safe (Z<=0): Acc = 0.7949 ± 0.1747 (avg count = 30.6)
            Warning (0<Z<=0.5): Acc = 0.7348 ± 0.1482 (avg count = 45.5)
            Critical (Z>0.5): Acc = 0.6551 ± 0.1227 (avg count = 35.9)
            """

            generate_zscore_accuracy_no_border_plot_1(os.path.join("./visualization/figures", "memory_buffer_z_score_new"))